...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs=3,
        metavar=("B", "G", "R"),
        default=(0, 255, 0),  # default mask color: green
        help="BGR color for overlay mask"
    )
...
def main():
    ...
    # Initialize predictor with the first frame and the selected ROI
    ...
    while True:
        ...
        # Build a colored overlay mask
        h_f, w_f = frame.shape[:2]
        overlay_mask = np.zeros((h_f, w_f, 3), dtype=np.uint8)
        for logit in mask_logits:
            binary = (logit[0] > 0.0).cpu().numpy().astype(np.uint8)
            # paint the mask area with chosen color
            overlay_mask[binary == 1] = mask_color_bgr

        # Blend mask over frame
        overlay = cv2.addWeighted(frame, 1.0, overlay_mask, 0.5, 0)

        cv2.imshow("SAM2 Interactive Tracker", overlay)
        key = cv2.waitKey(1) & 0xFF
        if key == ord('q'):
            break
        elif key == ord('r'):
            # Reset and select a new ROI
